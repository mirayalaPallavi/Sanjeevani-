...
export default function Auth() {
    ...
    try {
      ...
      if (role === 'doctor' && (!licenseNumber || !specialization)) {
        toast({
          ...
          description: t('auth.license_specialization_req'),
          variant: 'destructive',
        });
        return;
      }
      ...
      const metadata =
        role === 'doctor'
          ? { license_number: licenseNumber, specialization }
          : undefined;

      const { error } = await signUp(email, password, fullName, role as AppRole, metadata);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center gradient-hero p-4 relative overflow-hidden">
      {/* Decorative floating health-themed elements */}
      <div className="absolute -top-10 -left-10 w-72 h-72 bg-primary/10 rounded-full blur-3xl animate-float" />
      <div className="absolute -bottom-16 -right-10 w-80 h-80 bg-accent/10 rounded-full blur-3xl animate-float animation-delay-300" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-radial from-primary/5 to-transparent rounded-full" />

      <Card className="w-full max-w-md shadow-lg animate-fade-in relative z-10">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold">Sanjeevani</CardTitle>
          <CardDescription>
            {t('auth.trusted_platform')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="login">{t('auth.sign_in_title')}</TabsTrigger>
              <TabsTrigger value="signup">{t('auth.sign_up_title')}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="signup" className="space-y-4 mt-4">
              <form onSubmit={handleSignUp} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="signup-name">{t('auth.name_label')}</Label>
                  <Input
                    id="signup-name"
                ...
                {role === 'doctor' && (
                  <>
                    <div className="space-y-2">
                      <Label htmlFor="license">{t('auth.license_label')}</Label>
                      <Input
                        id="license"
                        placeholder={t('auth.license_placeholder')}
                        value={licenseNumber}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="specialization">{t('auth.specialization_label')}</Label>
                      <Select
                        value={specialization}
                        onValueChange={setSpecialization}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t('auth.specialization_placeholder')} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="General Practitioner">
                            {t('auth.specializations.gp')}
                          </SelectItem>
                          <SelectItem value="Cardiologist">
                            {t('auth.specializations.cardio')}
                          </SelectItem>
                          <SelectItem value="Dermatologist">
                            {t('auth.specializations.derm')}
                          </SelectItem>
                          <SelectItem value="Neurologist">
                            {t('auth.specializations.neuro')}
                          </SelectItem>
                          <SelectItem value="Pediatrician">
                            {t('auth.specializations.pedia')}
                          </SelectItem>
                          <SelectItem value="Psychiatrist">
                            {t('auth.specializations.psych')}
                          </SelectItem>
                          <SelectItem value="Orthopedist">
                            {t('auth.specializations.ortho')}
                          </SelectItem>
                          <SelectItem value="Gynecologist">
                            {t('auth.specializations.gyne')}
                          </SelectItem>
                          <SelectItem value="ENT Specialist">
                            {t('auth.specializations.ent')}
                          </SelectItem>
                          <SelectItem value="Ophthalmologist">
                            {t('auth.specializations.ophthal')}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {t('auth.note_admin_approval')}
                    </p>
                  </>
                )}
